...
async function _callGeminiAPI(body) {
  ...
  const res = await fetch('/api/gemini', {
    ...
    body: JSON.stringify({ body: body, model: 'gemini-1.5-flash' })
  });
  if (!res.ok) {
    const errData = await res.json().catch(() => ({}));
...
function _ensureAiStyles() {
  ...
  style.textContent = [
    ...
    '#scan-loading .ai-load-title { font-size:16px;font-weight:700;color:#fff; }',
    '#scan-loading .ai-load-sub { font-size:13px;color:rgba(255,255,255,0.6); }',
